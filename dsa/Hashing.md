...
**Intuition:** Imagine a library with millions of books. Finding a book by checking each shelf takes forever (O(n))! But with a catalog system that tells you EXACTLY which shelf (hash function â†’ index), you find it instantly (O(1))! That's **hashing** - converting data into an index/address for super-fast access.
...
- **Compilers** - Symbol tables (variable names â†’ memory)
...
"apple" â†’ hashCode() â†’ 93029210 â†’ 93029210 % arraySize â†’ index
```
...
map.get("apple");                 // Get - O(1) â†’ returns 5
map.containsKey("apple");         // Check key - O(1) â†’ true
map.containsValue(5);             // Check value - O(n) âš ï¸
...
set.contains("apple");            // Check - O(1) â†’ true
...
```
HashMap Internal:
Index 0: [key1â†’val1] â†’ [key7â†’val7]  (collision chain)
Index 1: null
Index 2: [key2â†’val2]
Index 3: [key3â†’val3] â†’ [key9â†’val9]  (collision chain)
...
```
...
- When size > capacity * loadFactor â†’ **rehashing** (doubles size)
...
@Override
public int hashCode() {
    // Must be consistent: equals() objects â†’ same hashCode
}
...
// Time: O(n), Space: O(n)
// Much better than nested loop: O(nÂ²)!
```
...
**Intuition:** Use HashMap to store **value â†’ last index**. When seeing duplicate, check distance!  
**Java:**
```java
boolean containsNearbyDuplicate(int[] nums, int k) {
    Map<Integer, Integer> map = new HashMap<>();  // value â†’ last index
...
**Intuition:** Need **bidirectional mapping**: s â†’ t AND t â†’ s. Use two HashMaps!  
**Java:**
```java
...
// Example:
// "egg", "add" â†’ true (eâ†’a, gâ†’d)
// "foo", "bar" â†’ false (o maps to both a and r)
```
...
**Intuition:** Same as isomorphic! Map pattern char â†” word bidirectionally.  
**Java:**
```java
...
### Problem 9: Subarray Sum Equals K â­â­â­
...
i=1, num=2: sum=3, map={0:1, 1:1, 3:1}, check (3-3=0) exists â†’ count=1
i=2, num=3: sum=6, map={0:1, 1:1, 3:1, 6:1}, check (6-3=3) exists â†’ count=2

Answer: 2 subarrays â†’ [3], [1,2]
```
...
**Intuition:** Similar to count! Map stores **prefix sum â†’ first index**. For each position, if (currentSum - k) exists, calculate length!  
**Java:**
```java
...
int countSubarrays(int[] arr) {
    ...
    for (int num : arr) {
        currentSum += (num == 1) ? 1 : -1;  // Transform 0 â†’ -1
...
### Problem 14: Longest Consecutive Sequence â­â­â­
...
Check 1: is start (0 not in set)
  â†’ 1, 2, 3, 4 â†’ length = 4 âœ“

Check 2: not start (1 in set) â†’ skip
Check 3: not start (2 in set) â†’ skip
Check 4: not start (3 in set) â†’ skip

Check 100: is start (99 not in set)
  â†’ just 100 â†’ length = 1

Check 200: is start (199 not in set)
  â†’ just 200 â†’ length = 1
...
**Intuition:** **Sliding window + HashMap!** Map stores **char â†’ last index**. When duplicate found, move left pointer to right of last occurrence!  
**Java:**
```java
...
**Intuition:** **Sliding window!** Expand right, when distinct > K, shrink left. HashMap tracks **char â†’ count**.  
**Java:**
```java
...
### Problem 17: Minimum Window Substring â­â­â­
...
**Intuition:** **Split into two groups!** Map stores (nums1[i] + nums2[j]) â†’ count. Then check if -(nums3[k] + nums4[l]) exists!  
**Java:**
```java
...
**Intuition:** **Map blacklist to valid range!** Remap blacklisted numbers < N-len(blacklist) to valid numbers â‰¥ N-len(blacklist).  
**Java:**
```java
class Solution {
...
âŒ **Using containsValue() on HashMap**
```java
map.containsValue(5);  // O(n) - Slow!
```
âœ… Better: Maintain reverse map if needed

âŒ **Modifying HashMap while iterating**
```java
for (String key : map.keySet()) {
    map.remove(key);  // ConcurrentModificationException!
}
```
âœ… Use Iterator with remove() or collect keys first

âŒ **Not handling null carefully**
```java
map.get(key).toString();  // NullPointerException if key doesn't exist!
```
âœ… Use getOrDefault() or check containsKey()

âŒ **Forgetting to override hashCode/equals**
```java
class Person { String name; }
Map<Person, String> map = new HashMap<>();  // Won't work correctly!
```
âœ… Override both hashCode() and equals()
...
```
Need to count/track frequency? â†’ HashMap<Element, Count>
Need to check existence? â†’ HashSet
Need order preserved? â†’ LinkedHashMap/LinkedHashSet
Need sorted order? â†’ TreeMap/TreeSet
Need to map both ways? â†’ Two HashMaps
Need to track index? â†’ HashMap<Element, Index>
```
