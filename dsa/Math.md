...
**Intuition:** Check from 1 to N. Optimization: check only till âˆšN because divisors come in pairs.  
...
**Intuition:** Check divisibility from 2 to âˆšN. If any number divides, not prime.  
**Logic:** Prime has exactly 2 divisors. Check till âˆšN for efficiency.
...
**Logic:** Divide by 2 till possible, then check odd numbers till âˆšN.
...
**Intuition:** When m is prime, use Fermat's Little Theorem: a^(m-1) â‰¡ 1 (mod m), so a^(-1) = a^(m-2) mod m.  
...
**Intuition:** If âˆšN is integer, then N is perfect square.  
...
**Intuition:** Trailing zeros = number of times 10 divides n!. Since 10 = 2Ã—5 and 2s are abundant, count 5s.  
...
**Intuition:** Use segmented sieve. Mark composites in range [L, R] using primes up to âˆšR.  
**Logic:** Find all primes up to âˆšR, use them to mark composites in [L, R].
...
// Numbers coprime with 12: 1, 5, 7, 11 â†’ count = 4
```
