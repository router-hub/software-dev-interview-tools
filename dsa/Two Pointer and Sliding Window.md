...
**Intuition:** Instead of checking all subarrays (O(n²)), use two pointers to efficiently expand and shrink windows based on conditions.
...
**Intuition:** Sliding window - count zeros. When zeros > k, shrink from left until zeros ‰¤ k.
...
**Intuition:** In valid window: length - maxFrequency ‰¤ k (can replace remaining chars).
...
String minWindowSubsequence(String s, String t) {
 ...
 while (sIdx < sLen) {
 int tIdx = 0;
 // No need to track where we started expanding!

 // Phase 1: Expand - find any complete match
 ...
 // Phase 2: Contract - find tightest match ending at matchEnd
 int matchEnd = sIdx - 1;
 sIdx = matchEnd;
 tIdx = tLen - 1;
 ...
 sIdx++; // Now sIdx points to the contracted window start
 int windowStart = sIdx; // ✓ This is the ONLY relevant start

 // Update minimum
 int windowLen = matchEnd - windowStart + 1;
 if (windowLen < minLen) {
 minLen = windowLen;
 minStart = windowStart;
 }

 sIdx = windowStart + 1; // Continue search
 }
