...
void solve( ArrayList<ArrayList<Integer>> adj) {
    int components = 0;
...
```java
void bfs(ArrayList<ArrayList<Integer>> adj) {
...
**Question:** Linear ordering where u before v for every edge u->v.
...
**Logic:** Track (node, cost, stops). Process if stops <= K.
...
**Intuition:** Need all-pairs distances -> Floyd-Warshall. Then count reachable cities for each.
**Logic:** Apply Floyd-Warshall, count distances <= threshold for each city.
...
**Logic:** Count components using DSU, check if extraEdges >= components-1.
...
**Logic:** DFS with tin[] and low[]. Check condition: low[child] >= tin[u].
