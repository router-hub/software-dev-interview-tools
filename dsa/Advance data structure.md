...
**Intuition:** Need (n-1) edges to connect n computers. Count: (1) number of components, (2) extra edges. If extra edges ≥ components - 1, possible!
**Java:**
```java
...
// Time: O(E log E) for sorting + O(E * α(V))
```

---

## DSU Problem 5: Number of Islands II (Dynamic Connectivity)
**Question:** Initially m×n grid is all water. Given list of positions to add land, return count of islands after each addition.
...
// Beats brute force: O(n) update + O(1) query → O(log n) + O(log n)
```
...
ft.update(1, 7); // Add 7 to index 1 (3 → 10)
...
**Intuition:** Divide array into ˆšn blocks. Store aggregate for each block. Query: full blocks + partial ends. Update: modify element + update its block.
**Java:**
```java
...
**Intuition:** Sort queries cleverly! Process in order that minimizes pointer movements. For queries in same ˆšn block, sort by right pointer.
**Java:**
```java
class Query {
 int l, r, idx;
...
|--------------|-------------|------|--------|
| **Union/Find, Connectivity** | DSU | O(α(n)) | Nearly constant operations |
...
| **Multiple Range Queries** | Mo's Algorithm | O((N+Q)ˆšN) | Offline, any range operation |
| **Simple Range Ops** | Sqrt Decomposition | O(ˆšn) | Easy to code |
...
### DSU Mistakes:
Œ Forgetting path compression
Œ Not using union by rank/size
œ… Always find() before comparing roots

### Segment Tree Mistakes:
Œ Wrong array size (use 4*n to be safe)
Œ Forgetting to update parent after child update
Œ Off-by-one in range queries
œ… Handle no-overlap/complete-overlap carefully

### Fenwick Tree Mistakes:
Œ Forgetting 1-indexed conversion
Œ Using for non-prefix operations
Œ Wrong last set bit calculation
œ… Remember: `i & (-i)` for last set bit
...
- Saves O(n) → O(log n) per range update
